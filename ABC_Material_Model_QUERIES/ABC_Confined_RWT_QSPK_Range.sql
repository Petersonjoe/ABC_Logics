...
DECLARE @iCOST_MONTH NVARCHAR(10) = '201610' -- used in line 814
DECLARE @MID_Q_DATE DATETIME

SET @MID_Q_DATE = CONVERT(DATETIME,'20160915') -- Changed every quarter
...
LEFT JOIN (
     ...
     WHERE [Record Status_Q] in('Valid' , 'Warning') 
	 AND CONVERT(VARCHAR(6),[Qspeak Upload Date_Q],112) <= @iCOST_MONTH + 2
...
SELECT 
  ...
  WHERE [DISP_LOCATION]<>'XXXX'
    AND [Report_Month] <= @iCOST_MONTH + 2
...
WHERE CONVERT(VARCHAR(6),ContainerReleaseDateTime,112) <= @iCOST_MONTH + 2
...
-- CC4 
IF OBJECT_ID(N'tempdb..#tmp_ccs_avg') IS NOT NULL
BEGIN
  TRUNCATE TABLE #tmp_ccs_avg
  DROP TABLE #tmp_ccs_avg
END
...
INTO #tmp_ccs_avg
--INTO rawdata.HPRT7_CCS_AVERAGE_USED
...
UPDATE AA
SET AA.[Credits] = CASE WHEN LOWER(AA.[Qspeak OEM Name]) = 'seagate'
                        AND ISNULL(AA.[Part Age], 1000.0) != 1000.0  
                        AND AA.[Part Age] <= 3.0
                        THEN BB.[Forecast] * -1.0
						WHEN LOWER(AA.[Qspeak OEM Name]) = 'seagate' 
						AND ISNULL(AA.[Part Age], 0.0) != 0.0 
						AND AA.[Part Age] > 3.0
						...
						THEN BB.[Forecast] * -1.0
						ELSE BB.[Forecast] * -1.0
					END,
	AA.[Credit Comments] = CASE WHEN LOWER(AA.[Qspeak OEM Name]) = 'seagate'
						        AND (ISNULL(AA.[Part Age], 1000.0) = 1000.0 
							    OR ISNULL(AA.[CT Code Label], '') = '')
								THEN 'CCS NB Fcst - Blank Part Age'
								ELSE 'CCS NB Fcst'
						   END
FROM [staging].[ABC_S3_LINK_SUPPLIER] AA, #tmp_ccs_avg BB
WHERE AA.[Physical Part Number] = BB.[Part No]
AND AA.[Region] = BB.Region
AND AA.[Received Month] = BB.[Month] 
AND AA.[Region] IN ('AME', 'EMEA')
...
-- blue step
-- limitation should be RRD & IW Credit
-- temp avg should use BIM IWC
IF OBJECT_ID(N'tempdb..#tmp_bimccs_avg') IS NOT NULL
BEGIN
  TRUNCATE TABLE #tmp_bimccs_avg
  DROP TABLE #tmp_bimccs_avg
END
...
FROM [staging].[ABC_S3_LINK_SUPPLIER] AA, #tmp_bimccs_avg BB
WHERE AA.[Physical Part Number] = BB.[Part No]
AND AA.[Region] = BB.Region
AND AA.[Received Month] = BB.[Month] 
AND AA.[L2 Supplier] = 'RRD'
AND AA.[Final Disp Flag] = 'IW Credit'
--AND AA.[Part Commodity] = 'Hard Drive'
...
AND AA.Region = 'APJ'
AND (
     AA.[L2 Supplier] != 'RRD' AND 
     ISNULL(AA.[L2 Supplier],'') not like'PCG%' AND 
	 LOWER(AA.[L2 Supplier]) != 'seagate'
	)
AND AA.[Final Disp Flag] = 'IW Credit'
...
SELECT [Region],
       [Event SPL],
	   [Unit Inv Cost] = (SELECT [Total Inv Cost]
						  FROM [fact].[vw_ABC_Inv_Cost_By_Region]
						  WHERE [Region] = AA.[Region]
						  AND [SPL] = AA.[Event SPL]
						  AND [Data Period] = @VERSION
						  )	   
